...
NUM_NAILS = int(args[200])  # ex: 300
MAX_ITERATIONS = int(args[4000])  # ex: 4000
NAILS_SKIP = 10
OUTPUT_TITLE = "output"
...
cx, cy = (BOARD_WIDTH/2/PIXEL_WIDTH, BOARD_WIDTH /
          2/PIXEL_WIDTH)  # center of circle
...
for i in range(MAX_ITERATIONS):
    ...
    for n in range(cur_nail+1+NAILS_SKIP, cur_nail+len(nails)-NAILS_SKIP):
        n = n % NUM_NAILS
        tmp_value = 0
        new_line = line(nails[cur_nail][0], nails[cur_nail]
                        [1], nails[n][0], nails[n][1])
    ...
    addLine.line((nails[cur_nail][0], nails[cur_nail][1],
                 nails[new_nail][0], nails[new_nail][1]), fill=0)
    res += " " + str(new_nail)
